...
void ReplayEngine::remove_actions(unsigned frame) {
    ...
    auto check_physics = [&](replay_data &action) -> bool { return action.frame > frame; };
    m_physicFrames_p1.erase(remove_if(m_physicFrames_p1.begin(), m_physicFrames_p1.end(), check_physics), m_physicFrames_p1.end());
    m_physicFrames_p2.erase(remove_if(m_physicFrames_p2.begin(), m_physicFrames_p2.end(), check_physics), m_physicFrames_p2.end());
}
