...
unsigned ReplayEngine::get_frame() {
    ...
    if (pl)
        return static_cast<unsigned>(pl->m_time() * hacks::tps);
    return 0;
} 
...
std::string ReplayEngine::save(std::string name)
{
    ...
    file.write(reinterpret_cast<char *>(&hacks::tps), sizeof(hacks::tps));
...
std::string ReplayEngine::load(std::string name)
{
    ...
    file.read(reinterpret_cast<char *>(&hacks::tps), sizeof(hacks::tps));
        ...
        if (ImGui::BeginTabBar("Recorder")) {
            ...
            if (ImGui::BeginTabItem("Audio")) {
                ...
                if (ImGui::Button("Save", {ImGui::GetContentRegionAvail().x, NULL})) {
                    ...
                    std::string command = "GDH/ffmpeg.exe -y -i fmodoutput.wav ";
                    if (!recorderAudio.extra_args.empty()) {
                        command += recorderAudio.extra_args + " ";
                    }

                    command += "\"GDH/recorder/" + audio_name_ + "\"";
            ...
            if (ImGui::BeginTabItem("Merge")) {
                ...
                if (ImGui::Button("Refresh", {ImGui::GetContentRegionAvail().x, NULL})) {
                    videos.clear();
                    audios.clear();
                    for (const auto &entry : std::filesystem::directory_iterator("GDH/recorder")) {
                ...
                if (ImGui::Button("Merge", {ImGui::GetContentRegionAvail().x, NULL})) {
                    if (videos.empty() || audios.empty()) {
                        ...
                        std::string command2 = "GDH/ffmpeg.exe -i \"" + videos[index_videos] + "\" -i \"" + audios[index_audios] + "\" -map 0:v -map 1:a -c:v copy ";
                        if (shortest) {
                            command2 += "-shortest ";
                        }
                        std::filesystem::path video_rel(videos[index_videos]);
                        command2 += "\"GDH/recorder/audio_" + video_rel.filename().string() + "\"";
            ...
            ImGui::Checkbox("Toggle", &sequence.enable_sqp, gui::scalePercentage);
            ImGui::SameLine();
            ImGui::Checkbox("Random", &sequence.random_sqp, gui::scalePercentage);
            ImGui::SameLine();
            int mode = (int)engine.mode;
            if (ImGui::RadioButton("Play", &mode, 2, gui::scalePercentage))
            ...
            if (ImGui::Button("Add"))
            {
                std::string path = "GDH/macros/" + sequence.replay_sq_name + ".re";
...
void SpamBot::handle_spambot(gd::PlayLayer *self)
{
    ...
    {
        hooks::GJBaseGameLayer_HandleButton_H(self, 0, downed, 1, player_p1);
    }
}
...
void StraightFly::handle_straightfly(gd::PlayLayer *self)
{
    ...
    {
        hooks::GJBaseGameLayer_HandleButton_H(self, 0, true, 1, true);
    }
    else if (accel > 0 && y > start_y - accel + accuracy / 100 && holding)
    {
        hooks::GJBaseGameLayer_HandleButton_H(self, 0, false, 1, true);
    }
}
