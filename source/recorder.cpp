...
void RenderTexture::capture_frame(std::mutex& lock, std::vector<uint8_t>& data, volatile bool& frame_has_data) {
    ...
    auto scene = gd::GameManager::sharedState()->getPlayLayer();
    scene->visit();
...
void Recorder::start() {
    ...
    std::thread([&, video_name_] {
        std::string command = "GDH/ffmpeg.exe -y -f rawvideo -pix_fmt rgb24 -s " +
        ...
        command += " -pix_fmt yuv420p -vf \"vflip\" \"GDH/recorder/" + video_name_ + "\"";
...
void Recorder::handle_recording(gd::PlayLayer* playLayer, float dt) {
    if (!has_level_complete_menu || after_end_extra_time < after_end_duration) {
        if (has_level_complete_menu) {
...
void RecorderAudio::start() {
    ...
    Console::WriteLn("Audio buffer recording started!");
}
...
void RecorderAudio::stop() {
    ...
    Console::WriteLn("Audio buffer recording stoped!");
}
